...
function makeInteractiveChart(id,data,color,isPsi,comparisonKey=""){
      ...
      const chartData=data.map(point=>({time:point.date,value:Number(point.value)}));series.setData(chartData);chart.timeScale().fitContent();box.__psdChartApi=chart;
      const observer=new ResizeObserver(entries=>entries.forEach(entry=>chart.applyOptions({width:Math.max(1,entry.contentRect.width),height:Math.max(210,entry.contentRect.height)})));observer.observe(box);chartCleanup.push(()=>{observer.disconnect();if(box.__psdChartApi===chart)delete box.__psdChartApi;chart.remove()});
