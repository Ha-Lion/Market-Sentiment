(function(){
  ...
  /* XX Ribbon Lab Version 2 — organized architecture.
     Markup/behavior live here. ALL ribbon appearance lives in xx-ribbon-lab.css. */
  ...
  const assetLinks = [
    ...
    { href: "precious-metals.html", icon: "◆", label: "Precious Metals Sentiment" },
  ...
  function enforceRibbonNamesAndAIStyle(){
    ...
    if(newsLink && newsLink.textContent.trim() !== "News & Articles") {
      newsLink.textContent = "News & Articles";
    }
  ...
  function ensureTourStyles(){
    /* Ribbon/tour appearance is owned by xx-ribbon-lab.css. */
  }
  ...
  function ensureAccountNavStyles(){
    /* Account/ribbon appearance is owned by xx-ribbon-lab.css. */
  }
  ...
  function labSavedTheme(){
    try{ return localStorage.getItem("psd-theme") === "dark" ? "dark" : "light"; }
    catch(error){ return "light"; }
  }

  function labApplyTheme(theme, save){
  ...
  function initializeRibbonThemeButton(){
    ...
    labApplyTheme(labSavedTheme(), false);
    syncLabel();

    button.addEventListener("click", function(){
      const next = document.body.classList.contains("dark-mode") ? "light" : "dark";
      labApplyTheme(next, true);
      syncLabel();
    });
  }
  ...
  <header class="header psd-shared-header">
    ...
    <nav class="nav" aria-label="Main navigation">
      <div class="psd-nav-row psd-nav-main">
        ${primaryLinks}
        ${accountLink}
        <button type="button" class="psd-theme-toggle" id="psd-ribbon-theme-toggle" aria-label="Switch between light and dark mode">${labSavedTheme() === "dark" ? "☀ Light mode" : "◐ Dark mode"}</button>
      </div>
    ...
    /* One alignment only, synchronously, while this parser-blocking lab script owns the ribbon. */
    alignRibbonGeometry();
  ...
  document.addEventListener("click", function(event){
    const link = event.target && event.target.closest ? event.target.closest('.psd-shared-header a[href="dashboard.html"]') : null;
    if(!link) return;
    const current = currentFile();
    if(current === "xx-dashboard-lab.html" || current === "xx-dashboard-lab2.html"){
      event.preventDefault();
      window.location.href = current === "xx-dashboard-lab.html" ? "xx-dashboard-lab2.html" : "xx-dashboard-lab.html";
    }
  }, true);
  ...
  /* This xx lab script sits immediately after #site-header, so render now. */
