...
(function(){
  ...
  function coreFormulaVersion(){
    return safeText((window.PSDCore && window.PSDCore.VERSION) || window.PSD_SENTIMENT_CORE_VERSION || "");
  }
  ...
  function sectionTitle(text, sub){
    ...
    const version = coreFormulaVersion();
    const sectionLabel = [text, sub, version ? `Formula ${version}` : ""].filter(Boolean).join(" • ");
