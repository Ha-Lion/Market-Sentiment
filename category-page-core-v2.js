...
function makeInteractiveChart(id,data,color,isPsi,comparisonKey=""){
      ...
      const chart=LightweightCharts.createChart(box,{width:box.clientWidth,height:Math.max(210,box.clientHeight||360),layout:{background:{type:LightweightCharts.ColorType.Solid,color:background},textColor:text,attributionLogo:false},grid:{vertLines:{color:grid},horzLines:{color:grid}},crosshair:{mode:LightweightCharts.CrosshairMode.Normal},timeScale:{borderColor:border,rightOffset:2},rightPriceScale:{borderColor:border,autoScale:true},handleScroll:{mouseWheel:true,pressedMouseMove:true,horzTouchDrag:true,
      vertTouchDrag:!window.matchMedia("(max-width:768px)").matches,
      vertTouchDrag:!window.matchMedia("(max-width:768px)").matches},handleScale:{axisPressedMouseMove:true,mouseWheel:true,pinch:true}});
...
async function fetchOptionalJson(url, timeoutMs=30000){
      ...
      try{
        if(!global.PSDDataV2?.fetchJson) return null;
        return await global.PSDDataV2.fetchJson(url,{required:false,cache:"no-cache",signal:controller?controller.signal:null});
      }catch(err){
        return null;
      }finally{
